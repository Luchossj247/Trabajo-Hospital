...
const ALL_NAV_ITEMS = [
  ...
  // Admin-only
  { name: 'Gestión de Personal',href: '/empleado/gestion-empleados', icon: Shield,          route: 'gestion-empleados',  section: 'admin' },
...
const ROLE_LABELS = {
  ...
  pharmacist: 'Farmacéutico', receptionist: 'Recepcionista',
}
...
export function MainLayout() {
  ...
  const employeeName = sessionStorage.getItem('employeeName') || 'Empleado'
  const employeeType = sessionStorage.getItem('employeeType') || ''

  useEffect(() => {
    if (!sessionStorage.getItem('employeeType')) navigate('/empleado-login')
  }, [navigate])

  const navItems = ALL_NAV_ITEMS.filter(item => hasAccess(employeeType, item.route))
  const mainItems = navItems.filter(i => i.section === 'main')
  const adminItems = navItems.filter(i => i.section === 'admin')

  const handleLogout = () => {
    sessionStorage.removeItem('employeeType')
    ...
    return (
      <Link
        ...
        className={`flex items-center px-3 py-2.5 rounded-xl text-sm font-semibold transition-all group
          ${isActive ? 'bg-[#ACEC00] text-[#013FF6] shadow-sm' : 'text-white/80 hover:bg-white/10 hover:text-white'}`}
      >
        <Icon className={`h-4.5 w-4.5 mr-3 flex-shrink-0 transition-colors ${isActive ? 'text-[#013FF6]' : 'text-white/70 group-hover:text-white'}`} style={{ width: '1.1rem', height: '1.1rem' }} />
      ...
      <aside className="w-60 flex-shrink-0 bg-[#013FF6] flex flex-col shadow-xl z-20">
          ...
          {adminItems.length > 0 && (
            <>
              <div className="pt-4 pb-1.5 px-3">
                <p className="text-[10px] font-bold uppercase tracking-widest text-white/30">Administración</p>
              </div>
        ...
        <div className="p-3 border-t border-white/10 space-y-0.5">
          <div className="px-3 py-2">
            <p className="text-xs font-semibold text-white/90 truncate">{employeeName}</p>
            <p className="text-[10px] text-white/40">{ROLE_LABELS[employeeType] || employeeType}</p>
          </div>
          <Link
            to="/empleado/perfil"
