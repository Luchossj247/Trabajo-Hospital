...
export function AdminDashboard() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const todayPatients = patients.filter(p => new Date(p.created_at).toDateString() === today)
      const freeBeds = bedList.filter(b => b.status === 'free' || b.status === 'libre').length
      const occupiedBeds = bedList.filter(b => b.status === 'occupied' || b.status === 'ocupada').length

      setStats({
        totalPatients: patients.length,
        todayPatients: todayPatients.length,
        totalEmployees: emps.length,
        activeEmployees: emps.filter(e => e.active !== false).length,
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-sm font-semibold text-slate-500">
              {stats.occupiedBeds} ocupadas · {stats.freeBeds} libres · {stats.totalBeds - stats.freeBeds - stats.occupiedBeds} en mantenimiento/limpieza
            </span>
          </div>
          ...
          <div className="flex gap-4 mt-2 text-xs text-slate-500">
            ...
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#ACEC00] inline-block"/> Libres</span>
            <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-slate-200 inline-block"/> Otro</span>
          </div>
        </div>
      )}
                  ...
                  <li key={p.id || i} className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                    <div className="w-9 h-9 rounded-full bg-[#013FF6]/10 flex items-center justify-center flex-shrink-0">
                      <span className="text-sm font-bold text-[#013FF6]">
                        {(p.nombre || p.name || 'P')[0].toUpperCase()}
                      </span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-slate-900 truncate">{p.nombre || p.name || 'Paciente'}</p>
                      <p className="text-xs text-slate-400 truncate">{p.dni || p.documento || p.email || '—'}</p>
                    </div>
                  ...
                  <li key={e.id || i} className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                    <div className="w-9 h-9 rounded-full bg-[#ACEC00]/20 flex items-center justify-center flex-shrink-0">
                      <span className="text-sm font-bold text-slate-700">
                        {(e.nombre || e.name || 'E')[0].toUpperCase()}
                      </span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-slate-900 truncate">{e.nombre || e.name || 'Empleado'}</p>
                      <p className="text-xs text-slate-400 truncate capitalize">{e.cargo || e.role || e.tipo || '—'}</p>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${e.active === false ? 'bg-slate-100 text-slate-400' : 'bg-[#ACEC00]/30 text-slate-700'}`}>
...
// ── Demo fallback data ────────────────────────────────────────────────────────
const DEMO_PATIENTS = [
  { id: 1, nombre: 'Carlos Méndez',   dni: '28.453.123', created_at: new Date().toISOString() },
  { id: 2, nombre: 'Ana Silva',       dni: '33.120.456', created_at: new Date().toISOString() },
  { id: 3, nombre: 'Pedro Gómez',     dni: '40.987.654', created_at: new Date().toISOString() },
  { id: 4, nombre: 'María López',     dni: '25.654.321', created_at: new Date().toISOString() },
  { id: 5, nombre: 'Juan Fernández',  dni: '37.223.789', created_at: new Date().toISOString() },
]

const DEMO_EMPLOYEES = [
  { id: 1, nombre: 'Dr. Ramírez',    cargo: 'doctor',        active: true },
  { id: 2, nombre: 'Enf. Torres',   cargo: 'enfermero',     active: true },
  { id: 3, nombre: 'Farm. Díaz',    cargo: 'farmacéutico',  active: true },
  { id: 4, nombre: 'Rec. García',   cargo: 'recepcionista', active: true },
  { id: 5, nombre: 'Dr. Varela',    cargo: 'doctor',        active: false },
]

const DEMO_BEDS = [
  { id: 'UTI-1', status: 'occupied' }, { id: 'UTI-2', status: 'occupied' },
  { id: 'UTI-3', status: 'free' },     { id: 'INT-1',  status: 'free' },
  { id: 'INT-2', status: 'occupied' }, { id: 'INT-3',  status: 'cleaning' },
  { id: 'PED-1', status: 'free' },     { id: 'PED-2',  status: 'occupied' },
]